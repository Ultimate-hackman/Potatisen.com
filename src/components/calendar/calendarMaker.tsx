...
const day: number = new Date().getDate() - monday;
...
function dayMaker(itemData, saturation, i, date, weekIndex, count, duplicate, state, duplicateChild, data) {
  ...
  return (
    <Hatch cursor={cursor} onClick={() => { if (count >= 2) { state(duplicate(i + day)); } }} color={colorFinder(color, saturation / 2)} key={i}>
      {" "}
      <Text weight={weight} size={sizes[0]}>
        {" "}
        {date[0]}
        {" "}
        {months[date[1]]}
      ...
      {" "}
      <Text weight={weight} size={sizes[1]}>
        {" "}
        {itemData[0]?.split(",")[1]}
        {" "}
        <Text size={sizes[1]}>{hours}</Text>
        {" "}
        {daysToGo}
        {" "}
        {weekDay}
...
function multiTest(data, language, ugg, weekIndex, i, state, saturation, len) {
  ...
  const current: number = new Date().getDate() + (new Date().getFullYear() * 365) + totalMonth(currentMonth);

  const filterData = [];
  data.forEach((item) => {
    const time = new Date((`${item[0]} `).split(",")[0]);

    const dataTime: number = time.getDate() + (time.getFullYear() * 365) + (totalMonth(time.getMonth()) + 1);

    const distance = dataTime - current;
